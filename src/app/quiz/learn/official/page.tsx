...
export default function OfficialLearnPage() {
  ...
  useEffect(() => {
    ...
    async function initializeLearn() {
      ...
      try {
        const response = await fetch(`/api/questions/learn/official`);

        if (!response.ok) {
          let errorMsg = `Failed to load official model questions`;
          try {
        ...
        if (data.length === 0) {
          throw new Error(`No official model questions found.`);
        }
        ...
        console.error("Error initializing official learn mode:", error);
            ...
            <div className="flex justify-between items-center mb-2">
              <div className="flex items-center gap-4">
                <span className="text-sm font-semibold text-foreground">
                  Learn Mode - Official Model Questions
                </span>
